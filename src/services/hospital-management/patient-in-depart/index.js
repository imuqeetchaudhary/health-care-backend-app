...
exports.findById = ({ id }) => {
    return db.PatientInDepart.findByPk(id, {
        attributes: ["patientInDepartId"],
        include: [{
                model: db.Department,
                attributes: ["departmentId", "departmentName"],
...
exports.updatePatientInDepart = async({
    ...
    try {
        const updatedPatientInDepart = await db.PatientInDepart.update({...patientInDepart }, { where: { patientId: patientInDepartId } });
...
exports.deletePatientInDepart = async({ patientInDepartId }) => {
    try {
        ...
        if (dbUtils.isFkFailed(err)) {
            throw new Exceptions.BaPatientequest({
                message: "Can't delete patient in department unless delete all its reference",
            });
        }
        throw err;
    }
