...
exports.getSingleQualificationByDr = promise(async(req, res) => {
    const { id } = req.params;

    const qualification = await qualificationService.findByDrId({ id });
    res.status(200).json({ qualification });
});
...
exports.deleteQualificationByDr = promise(async(req, res) => {
    const { id } = req.params;
    const qualificationId = id;

    const deleteQualification = await qualificationService.deleteQualificationByDr({
        qualificationId,
    });
    res.status(200).json({ message: "Successfully deleted qualification" });
});