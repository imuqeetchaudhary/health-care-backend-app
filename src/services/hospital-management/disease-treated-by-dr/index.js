...
exports.updateDiseaseTreatedByDr = async({
    ...
    try {
        const updatedDiseaseTreatedByDr = await db.DiseaseTreatedByDr.update({...diseaseTreatedByDr }, { where: { dtbdId: diseaseTreatedByDrId } });
...
exports.deleteDiseaseTreatedByDr = async({ diseaseTreatedByDrId }) => {
    try {
        const deleteDiseaseTreatedByDr = await db.DiseaseTreatedByDr.destroy({
            where: { dtbdId: diseaseTreatedByDrId },
        });
    } catch (err) {
        if (dbUtils.isFkFailed(err)) {
            throw new Exceptions.BadRequest({
                message: "Can't delete Disease Treated By Doctor unless delete all its reference",
            });
        }
        throw err;
    }
